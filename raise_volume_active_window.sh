...
# Define log file (commented out)
# LOG_FILE="$HOME/raise_volume_active_window.log"

# Log function (commented out)
# log() {
...
')
stream_index=$(echo "$stream_index" | tr -d '#')
...
if [ "$current_volume" -lt 100 ]; then
  ...
  if pactl set-sink-input-volume "$stream_index" '+10%' 2>/dev/null; then
  ...
  if [ "$updated_volume" -gt 100 ]; then
    ...
    if pactl set-sink-input-volume "$stream_index" 100% 2>/dev/null; then
